...
class HamburguesaViewSet(viewsets.ViewSet):
    ...
    @action(methods=['delete', 'put'], detail=True, url_path='ingrediente/(?P<oid>\w+)', url_name='busqueda')
    def hola(self,request,pk=None, oid=None):
        ...
        if request.method == 'DELETE':
            ...
            if not pk.isnumeric():
                ...
                return response
            if not oid.isnumeric():
                response = Response(status=400)
                response.reason_phrase= "Id de ingrediente inválido"
                return response
            try:
